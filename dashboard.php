        ...
        if (empty($addr_errors)) {
            try {
                ...
                if ($est_principale_livraison) {
                    ...
                    $stmt_unset_ship->bindParam(':id_user', $user_id, PDO::PARAM_INT);
                    $stmt_unset_ship->bindParam(':id_addr', $address_id, PDO::PARAM_INT);
                    $stmt_unset_ship->execute();
                }
                if ($est_principale_facturation) {
                    ...
                    $stmt_unset_bill->bindParam(':id_user', $user_id, PDO::PARAM_INT);
                    $stmt_unset_bill->bindParam(':id_addr', $address_id, PDO::PARAM_INT);
                    $stmt_unset_bill->execute();
                }
    ...
    if (!isset($_GET['csrf_token']) || !hash_equals($csrf_token, $_GET['csrf_token'])) {
        ...
        if ($address_id_to_delete > 0) {
            try {
                ...
                $stmt_check = $pdo->prepare("SELECT COUNT(*) as usage_count FROM Commandes WHERE id_adresse_livraison = :id_adresse OR id_adresse_facturation = :id_adresse");
                $stmt_check->execute([':id_adresse' => $address_id_to_delete]);
                $usage_result = $stmt_check->fetch();
                $usage_count = $usage_result['usage_count'];
                
                if ($usage_count > 0) {
                    $_SESSION['dashboard_message'] = ['type' => 'error', 'text' => 'Cette adresse ne peut pas être supprimée car elle est utilisée dans ' . $usage_count . ' commande(s).'];
                } else {
        ...
        if ($address_id_to_set > 0 && ($address_type === 'shipping' || $address_type === 'billing')) {
            try {
                ...
                if ($is_making_default) {
                    ...
                    $stmt_unset->bindParam(':id_user', $user_id, PDO::PARAM_INT);
                    $stmt_unset->bindParam(':id_addr', $address_id_to_set, PDO::PARAM_INT);
                    $stmt_unset->execute();
                }
...
// Vérifier l'utilisation de chaque adresse dans les commandes
$address_usage = [];
if (!empty($addresses)) {
    foreach ($addresses as $address) {
        try {
            $stmt_usage = $pdo->prepare("SELECT COUNT(*) as usage_count FROM Commandes WHERE id_adresse_livraison = :id_adresse OR id_adresse_facturation = :id_adresse");
            $stmt_usage->execute([':id_adresse' => $address['id_adresse']]);
            $usage_result = $stmt_usage->fetch();
            $address_usage[$address['id_adresse']] = $usage_result['usage_count'];
        } catch (PDOException $e) {
            error_log("Error checking address usage: " . $e->getMessage());
            $address_usage[$address['id_adresse']] = 0;
        }
    }
}
...
<head>
    ...
    <style>
        ...
        .address-actions a.disabled { color: #666; cursor: not-allowed; text-decoration: none; }
        .address-actions a.disabled:hover { text-decoration: none; }
        .address-used-indicator {
                            ...
                            <div id="address-list-container" class="<?php echo empty($addresses) ? 'placeholder-content' : ''; ?>">
                                <?php if (!empty($addresses)): ?>
                                    <?php foreach($addresses as $addr): 
                                        $is_used = isset($address_usage[$addr['id_adresse']]) && $address_usage[$addr['id_adresse']] > 0;
                                    ?>
                                    <div class="address-card <?php echo $is_used ? 'address-used' : ''; ?>" data-address-id="<?php echo sanitize_html_output($addr['id_adresse']); ?>">
                                        <p><strong><?php echo sanitize_html_output(!empty($addr['type_adresse']) ? $addr['type_adresse'] : 'Adresse'); ?>
                                        <?php if ($is_used): ?>
                                            <span class="address-used-indicator">🔒 Utilisée</span>
                                        <?php endif; ?>
                                        </strong><br>
                                        ...
                                        <div class="address-actions">
                                            ...
                                            <?php if ($is_used): ?>
                                                <span class="address-action-btn disabled">Verrouillée</span>
                                            <?php else: ?>
