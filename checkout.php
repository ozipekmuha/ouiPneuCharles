...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['action']) && $_POST['action'] == 'add_address') {
        ...
        $type_adresse = trim($_POST['type_adresse'] ?? 'Autre'); 
        ...
        if (empty($errors_checkout)) {
            ...
            $db_shipping_ht = round($db_shipping_ttc / (1 + $TVA_RATE), 2); 
...
<body>
                    ...
                    <div class="checkout-layout">
                                ...
                                <div id="add-new-address-form-checkout" style="display:none;">
                                    ...
                                    <form method="POST" action="checkout.php"> 
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="destinataire_nom_complet" id="destinataire_nom_complet">
                                        </div>
                                        <div class="form-group">
                                            <label for="adresse_ligne1">Adresse Ligne 1</label>
                                            <input type="text" name="adresse_ligne1" id="adresse_ligne1">
                                        </div>
                                        <div class="form-group">
                        ...
                        <aside class="checkout-sidebar">
                            <section class="checkout-section" id="checkout-summary">
                                ...
                                <?php endif; ?>
                                <button id="submit" class="cta-button" style="width:100%; margin-top:1.5rem;" <?php if(empty($cart_items_details_display) || empty($user_addresses_display)) echo 'disabled';?>>
                                    <span id="button-text">Confirmer la commande</span>
                                </button>
                                <?php if(empty($user_addresses_display)): ?>
    ...
    <script>
        ...
        async function initialize() {
            const shipping_address_id = document.querySelector('input[name="shipping_address_id"]:checked').value;
            const billing_same_as_shipping = document.querySelector('#billing_same_as_shipping').checked;
            const billing_address_id = billing_same_as_shipping ? shipping_address_id : document.querySelector('input[name="billing_address_id"]:checked').value;
        ...
        async function handleSubmit(e) {
            ...
            const { error } = await stripe.confirmPayment({
                elements,
                confirmParams: {
                    return_url: "<?php echo 'http://' . $_SERVER['HTTP_HOST'] . '/payment_success.php'; ?>",
                },
            });
